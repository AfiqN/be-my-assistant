...
*(Tempatkan GIF atau screenshot utama di sini)*

*(Contoh Caption: Customer asking about company services and receiving a context-based answer)*
...
*(Tempatkan screenshot fitur-fitur utama lainnya di sini, seperti Admin Dashboard, Upload, Persona Config, Context Test)*
...
4. **Set Up Environment Variables:**
   ...
   * **Edit** the `.env` file and **add your Google Generative AI API Key**:
     ...
     # Optional: Override default model names or paths if needed
...
* `EMBEDDING_MODEL_NAME`: Specifies the Sentence Transformer model used for creating text embeddings (defaults to `paraphrase-multilingual-MiniLM-L12-v2`).
* `LLM_MODEL_NAME`: Specifies the Google Generative AI model used for generating chat responses (defaults to `gemini-1.5-flash`).
...
* `RAG_TEMPERATURE`: Controls the creativity/randomness of the LLM's responses. Lower values (e.g., 0.2) are more deterministic, higher values (e.g., 0.9) are more creative (defaults to 0.7).
