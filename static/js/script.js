document.addEventListener('DOMContentLoaded', function() {
    ...
    function initViewSwitcher() {
      ...
      Elements.nav.adminBtn.addEventListener('click', () => switchView('admin-view'));
      Elements.nav.customerBtn.addEventListener('click', () => switchView('customer-view'));
    ...
    function switchView(viewId) {
      ...
      if (viewId === 'admin-view') {
        ...
        if (Elements.header.title) Elements.header.title.textContent = 'Admin Dashboard';
        if (Elements.header.subtitle) Elements.header.subtitle.textContent = 'Manage AI Assistant Context';
      } else {
        Elements.nav.customerBtn.classList.add('active');
        if (Elements.header.title) Elements.header.title.textContent = 'Customer View';
        if (Elements.header.subtitle) Elements.header.subtitle.textContent = 'AI Powered Customer Service';
      }
    ...
    function initAdminFileUpload() {
      ...
      if (!uploadForm) {
        console.warn("Admin upload form not found. Skipping admin event listeners.");
        return;
      }
    ...
    function handleFileSelection() {
      ...
      if (fileInput.files.length > 0) {
        ...
        console.log("Admin: Valid file selected. currentFile set to:", AppState.admin.currentFile);
      } else {
    ...
    function handleUploadSubmit(e) {
      ...
      showAdminStatus('<div class="spinner-border spinner-border-sm me-2" role="status"></div> Processing document...', 'loading');
      uploadBtn.disabled = true;
      ...
      .then(response => {
        ...
        if (!response.ok) {
          return response.json().then(errData => {
            ...
            console.error("Admin: Failed to parse backend error JSON or generic HTTP error:", jsonError);
      ...
      .catch(error => {
        ...
        showAdminStatus(`<i class="bi bi-exclamation-triangle"></i> Error: ${error.message}`, 'error');
      })
      .finally(() => {
    ...
    function showAdminStatus(message, type) {
      ...
      statusDiv.className = `status-message ${type} ${autoClear ? 'fade-in' : ''}`;
    ...
    function updateDocumentList() {
      ...
      if (AppState.admin.processedFiles.length === 0) {
        documentList.innerHTML = '<li class="list-group-item text-muted"><em>No processed documents yet.</em></li>';
      } else {
        AppState.admin.processedFiles.forEach(filename => {
          ...
          li.innerHTML = `
            <div class="d-flex justify-content-between align-items-center">
              ...
              <button class="btn btn-sm btn-outline-danger delete-doc-btn" data-filename="${escapeHTML(filename)}">
        ...
        documentList.querySelectorAll('.delete-doc-btn').forEach(btn => {
          btn.addEventListener('click', function() {
            ...
            AppState.admin.processedFiles = AppState.admin.processedFiles.filter(f => f !== filename);
            updateDocumentList();
            showAdminStatus(`<i class="bi bi-check-circle"></i> Document "${filename}" removed.`, 'success');
          });
        });
      }
    }
    ...
    function initCustomerChat() {
        const { chatForm, clearChatBtn, chatbox, loadingIndicator } = Elements.customer;
        
        if (!chatForm) {
          console.warn("Customer chat form not found. Skipping customer event listeners.");
          return;
        }
        
        // Ensure loading indicator is in the chatbox
        if (loadingIndicator && chatbox && loadingIndicator.parentNode !== chatbox) {
    ...
    function handleChatSubmit(e) {
        ...
        const { questionInput, sendButton, chatbox, loadingIndicator, welcomeMessage } = Elements.customer;
        ...
        .then(response => {
            ...
            return response.json().then(errData => {
                ...
                console.error("Customer: Failed to parse backend error JSON or generic HTTP error:", jsonError);
                throw new Error(`HTTP error! status: ${response.status}`);
            });
            }
        ...
        .catch(error => {
            ...
            addBotMessage(`Sorry, there was an issue: ${error.message}. Please try asking again.`);
        })
        .finally(() => {
            // Hide loading state
    ...
    function addBotMessage(text) {
      ...
      AppState.chat.history.push({ role: 'assistant', content: text, time: time });
